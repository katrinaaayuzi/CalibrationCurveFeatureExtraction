...
filePath = 'D:\Codes\Curve feature extration\SAcurve\different HC surfactants\'; % Replace with the actual path to your Excel file
filename = [filePath, 'HCsurf-Capstone-MilliQ-SA.xlsx']; % Construct the filename dynamically
sheetName = 'Sheet1'; % Assuming the sheet name is always 'Sheet1'
% Analyte = 'MilliQ'; % MilliQ, Volvic, Evian, Aquamia, Gerolsteiner, Adelholzener, Spree, BrandenburgerQuell, Vio

% Construct the column names dynamically based on HCsurf
x_column = ['f_conc']; % e.g., 'f_Triton'
...
export_filename = [HCsurf,'-Capstone-MilliQ-SA'];
...
transition_x2_start = 0.04; % Actual x value where the second linear region starts
transition_x2_end = 0.36; % Actual x value where the second linear region ends
