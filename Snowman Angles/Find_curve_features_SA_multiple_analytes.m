...
filePath = 'D:\Codes\Curve feature extration\KS_158\'; % Replace with the actual path to your Excel file
filename = [filePath, 'SDBS-Capstone-Analytes-SA.xlsx']; % Construct the filename dynamically
sheetName = 'Sheet1'; % Assuming the sheet name is always 'Sheet1'

% Construct the column names dynamically based on HCsurf
x_column = ['c_', HCsurf]; % e.g., 'f_Triton'
...
export_filename = [HCsurf, '-Capstone-Analytes-SA'];
...
for i = 1:length(analyte_columns)
    ...
    xlabel(sprintf('c(%s)', HCsurf)); ylabel('Snowman angle');
...
% Define headers
headers = {'Analyte', 'Min Concentration', 'Min Snowman Angle', 'Max Concentration', 'Max Snowman Angle', 'Concentration Difference', 'Angle Difference'};
